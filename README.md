...
- I previously made a GUI version of this utility in Flutter, but at the moment the FlutterMidiCommand package it uses for the backend has bugs on Linux. However I want to keep that version "as is" with the package (instead of replacing it with a FFI backend), as there is the option of easily porting to other platforms in the future should the MIDI package be picked up and maintained again. Flutter continues to be my primary GUI library choice, and the Flutter version can still considered maintained and used instead.
...
IUP's precompiled library and include headers are expected to be present under the lib/iup directory. It is not checked into git so you will need to add it.
...
This limitation can be removed but I didn't fancy implementing the trickier code it would require!

I also have no clue how if this works with the extra locations in the XL/XL+, e.g. how presets after 383 work or IRs after 100.
